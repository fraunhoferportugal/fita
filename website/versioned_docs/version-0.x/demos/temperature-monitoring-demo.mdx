...
```shell
docker container run --name embserve_nodes --network host --privileged -v /dev/pts/:/dev/pts/ -d ghcr.io/fraunhoferportugal/fita/components/iotnetemu/embserve_nodes:latest
