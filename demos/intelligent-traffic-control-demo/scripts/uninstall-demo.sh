...
microk8s kubectl delete $(microk8s kubectl get deployments -o name | grep noise-deployment) > /dev/null 2>&1
microk8s helm uninstall sensor-consumer
microk8s helm uninstall video-component
microk8s helm uninstall fita-tig
microk8s helm uninstall fita-metrics
microk8s helm uninstall fita


# Remove Intel GPU plugin
pushd $DIR
./delete-intel-gpu-plugin.sh 2> /dev/null
popd
